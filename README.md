...
## Why trees?
My passion is studying how the built environment impacts human health. Trees, which are amazing in nature, are integrally tied to the built environment in urban areas and have major impacts on our health. If you are also into this stuff, follow me at [Well Placed](https://www.facebook.com/WellPlacedPage/) on Facebook. Learn more:
...
7. On the upper right is a tab with a chart icon that says 'Show Me'. Click this icon to reveal common chart types available for you to build and what minimum data requirements exist for each. Click 'Show Me' again to hide it and get it out of the way.
...
10. Add a filter. Drag 'Town' (under 'Dimensions') into the 'Filters' box. In the pop-up, select the 'Use all' or 'All' button, and click 'OK' for Windows or the red X for Mac. This filter will come into play once we create a dashboard.
...
15. On the lower left, just to the right of the `Plantable Space Percentage` tab, there's an icon that includes a tiny bar chart and a plus sign. Click that icon to create a new worksheet.
16. From under 'Dimensions' on the left panel, drag 'Town' to the middle of the center panel. It may take a few seconds to associate the field data to pre-loaded location data.
...
18. Pan over to Chicago and zoom in so you can see all the dots over Chicago area communities. You'll notice that not all communities were appropriately associated with the map since their names are too generic and Tableau didn't know where to place them (sorry, Plainfield). Fortunately, we still retain about half the towns which should be enough for some geographic exploration.
...
19. Add a couple of filters for our dashboard.
...
20. Add some meaningful color. Drag 'Acres' (under 'Measures') over the 'Color' icon in the 'Marks' box. Edit the colors to better see how much canopy availability differs by community. This will keep the focus on where there is potential land to plant trees.
...
21. Change the dot sizes to reflect acreage of plantable space in each community, by land use type. To do so, drag 'Acres' (under 'Measures') over the 'Size' icon in the 'Marks' box.

22. On the lower left, right-click the `Sheet 2` tab and click 'Rename'. Type `Plantable Acreage by Community`.

### Create dashboard

23. On the lower left, just to the right of your two worksheet tabs, there are three small icons. Click the middle icon, which includes a tiny 2x2 grid and a plus sign, to create a new dashboard.
24. Notice that for dashboards the left panel has different components than you saw previously for the worksheets. One of these is 'Size'.
...
25. Add your two worksheets to this dashboard.
...
26. Define that you want to filter the dashboard by community.
